...
def load_Waterbirds(datapath, batch_size, imagesize, logfile):
    wbdata = WaterbirdsDataset(datapath, imagesize)
...
def load_celeba(datapath, batchsize, imagesize, logfile, validation_split=0.1, max_threads=10):
    celebadata = CelebADataset(datapath, imagesize)
    splits = ['train', 'val', 'test']
    subsets = celebadata.get_splits(splits)
    # subsets = [DRODataset(subsets[split], process_item_fn=None, n_groups=celebadata.n_groups,
    #                           n_classes=celebadata.n_classes, group_str_fn=celebadata.group_str) \
    #                for split in splits]
    
    log(logfile, ("preparing data"))
    train_data, val_data, test_data = subsets['train'], subsets['val'], subsets['test']
