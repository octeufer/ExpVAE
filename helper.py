...
def get_transform_cub_bak(imagesize):
    scale = 256.0/imagesize
    target_resolution = (imagesize, imagesize)
    center_transforms= [
...
def get_transform_cub(imagesize):
...
class WaterbirdsDataset(Dataset):
    ...
    def __init__(self, datapath, imagesize):
        ...
        self.train_transform = get_transform_cub(imagesize)
        self.eval_transform = get_transform_cub(imagesize)
...
class CelebADataset(Dataset):
    ...
    def __init__(self, datapath, imagesize):
        ...
        self.data_dir = datapath
        # self.target_name = args.target_name
        self.confounder_names = 'Eyeglasses'
        ...
        self.splitdata_df = pd.read_csv(
            os.path.join(self.data_dir, 'list_eval_partition.csv')
        )

        # Get the y values
        self.y_array = self.metadata_df['Male'].values
        self.n_classes = 2

        # We only support one confounder for CUB for now
        self.confounder_array = self.metadata_df['Eyeglasses'].values
        self.n_confounders = 1

        self.proxy_array = self.metadata_df['Smiling'].values
        self.proxy_array = torch.LongTensor(self.proxy_array)
        ...
        self.split_array = self.splitdata_df['partition'].values
        self.split_dict = {
            'train': 0,
            'val': 1,
            'test': 2
        ...
        # self.ID_array = self.metadata_df['img_filename'].str.split(".").str.get(0).astype(int)
        # self.y_array = torch.LongTensor(self.y_array)
        # self.confounder_array = torch.LongTensor(self.confounder_array)
    ...
    def __getitem__(self, idx):
        ...
        # ID = self.ID_array[idx]
        img_filename = os.path.join(
            self.data_dir, 'img_align_celeba',
            self.filename_array[idx])
        img = Image.open(img_filename).convert('RGB')
        if self.split_array[idx] == self.split_dict['train'] and self.train_transform:
            img = self.eval_transform(img)
    ...
    def get_splits(self, splits):
        ...
        for split in splits:
            ...
            subsets[split].y_array = self.y_array[indices]
            subsets[split].group_array = self.group_array[indices]
            subsets[split].confounder_array = self.confounder_array[indices]
            subsets[split].proxy_array = self.proxy_array[indices]
            subsets[split].filename_array = self.filename_array[indices]
        return subsets
